...
func (suite *SelectionSuite) TestCases() {
    ...
    var tbl2Name r.Term = tbl2.Info().Field("name")
    ...
    var tbl2DbName r.Term = tbl2.Info().Field("db").Field("name")
    ...
    {
        // selection.yaml line #115
        /* err("ReqlNonExistenceError", 'Expected type STRING but found NULL.') */
        var expected_ Err = err("ReqlNonExistenceError", "Expected type STRING but found NULL.")
        /* r.db(tbl2DbName).table(tbl2Name, read_mode=null).count() */

    	suite.T().Log("About to run line #115: r.DB(tbl2DbName).Table(tbl2Name, r.TableOpts{ReadMode: nil, }).Count()")

        runAndAssert(suite.Suite, expected_, r.DB(tbl2DbName).Table(tbl2Name, r.TableOpts{ReadMode: nil, }).Count(), suite.session, r.RunOpts{
			GeometryFormat: "raw",
    	})
    ...
    var nested r.Term = r.Expr([]interface{}{[]interface{}{1, 2}, []interface{}{3, 4}, []interface{}{5, 6}})
