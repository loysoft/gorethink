...
func (suite *ChangefeedsIdxcopySuite) SetupTest() {
	fmt.Println("Setting up ChangefeedsIdxcopySuite")
...
func (suite *ChangefeedsIdxcopySuite) TearDownSuite() {
	fmt.Println("Tearing down ChangefeedsIdxcopySuite")
...
func (suite *ChangefeedsIdxcopySuite) TestCases() {
	fmt.Println("Running ChangefeedsIdxcopySuite: Test duplicate indexes with squashing")
...

	{
		...
		fmt.Println("About to run line #4: tbl.IndexCreate('a')")
...

	{
		...
		fmt.Println("About to run line #6: tbl.IndexWait('a')")
	...
	fmt.Println("Possibly executing: var feed r.Term = tbl.OrderBy(r.OrderByOpts{Index: 'a', }).Limit(10).Changes(r.ChangesOpts{Squash: 2, }).Limit(9)")
...

	{
		...
		fmt.Println("About to run line #15: tbl.Insert(r.Range(0, 12).Map(map[interface{}]interface{}{'id': r.Row, 'a': 5, }))")
...

	{
		...
		fmt.Println("About to run line #20: tbl.GetAllByIndex('id', 1, 8, 9).Delete()")
...

	{
		...
		fmt.Println("About to run line #26: wait(2)")
...
{"new_val":{"a":5, "id":11}, "old_val":nil}]) */
		...
		fmt.Println("About to run line #28: fetch(feed, 0)")
