...
func (suite *ChangefeedsIncludeStatesSuite) TestCases() {
...

    {
    	...
    	suite.T().Log("About to run line #4: tbl.Changes().Limit(1)")

        cursor, err := tbl.Changes().Limit(1).Run(suite.session, r.RunOpts{
			GeometryFormat: "raw",
    	})
...

    {
    	...
    	suite.T().Log("About to run line #9: tbl.Get(0).Changes().Limit(3)")

        cursor, err := tbl.Get(0).Changes().Limit(3).Run(suite.session, r.RunOpts{
			GeometryFormat: "raw",
    	})
...

    {
    	...
    	suite.T().Log("About to run line #14: tbl.OrderBy().Limit(10).Changes().Limit(2)")

        cursor, err := tbl.OrderBy().Limit(10).Changes().Limit(2).Run(suite.session, r.RunOpts{
			GeometryFormat: "raw",
    	})
...

    {
        ...
        cursor, err := tbl.Insert(map[interface{}]interface{}{"id": 1, }).Run(suite.session, r.RunOpts{
			GeometryFormat: "raw",
    	})
...

    {
    	...
    	suite.T().Log("About to run line #21: tbl.OrderBy().Limit(10).Changes().Limit(3)")

        cursor, err := tbl.OrderBy().Limit(10).Changes().Limit(3).Run(suite.session, r.RunOpts{
			GeometryFormat: "raw",
    	})
    ...
    suite.T().Log("Possibly executing: var tblchanges r.Term = tbl.Changes()")

    var tblchanges r.Term = tbl.Changes()
...

    {
        ...
        cursor, err := tbl.Insert(map[interface{}]interface{}{"id": 2, }).Run(suite.session, r.RunOpts{
			GeometryFormat: "raw",
    	})
...

    {
        ...
        fetch(suite.Suite, suite.session, expected_, tblchanges, 2, r.RunOpts{
			GeometryFormat: "raw",
    	})
    ...
    suite.T().Log("Possibly executing: var getchanges r.Term = tbl.Get(2).Changes()")

    var getchanges r.Term = tbl.Get(2).Changes()
...

    {
        ...
        cursor, err := tbl.Get(2).Update(map[interface{}]interface{}{"a": 1, }).Run(suite.session, r.RunOpts{
			GeometryFormat: "raw",
    	})
...

    {
        ...
        fetch(suite.Suite, suite.session, expected_, getchanges, 4, r.RunOpts{
			GeometryFormat: "raw",
    	})
    ...
    suite.T().Log("Possibly executing: var limitchanges r.Term = tbl.OrderBy().Limit(10).Changes()")

    var limitchanges r.Term = tbl.OrderBy().Limit(10).Changes()
    ...
    suite.T().Log("Possibly executing: var limitchangesdesc r.Term = tbl.OrderBy().Limit(10).Changes()")

    var limitchangesdesc r.Term = tbl.OrderBy().Limit(10).Changes()
...

    {
        ...
        cursor, err := tbl.Insert(map[interface{}]interface{}{"id": 3, }).Run(suite.session, r.RunOpts{
			GeometryFormat: "raw",
    	})
...

    {
        ...
        fetch(suite.Suite, suite.session, expected_, limitchanges, 5, r.RunOpts{
			GeometryFormat: "raw",
    	})
...

    {
        ...
        fetch(suite.Suite, suite.session, expected_, limitchangesdesc, 5, r.RunOpts{
			GeometryFormat: "raw",
    	})
