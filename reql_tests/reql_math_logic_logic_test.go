...
func (suite *MathLogicLogicSuite) TestCases() {
...

    {
        ...
        cursor, err := r.And(true, r.Expr(true)).Run(suite.session, r.RunOpts{
			GeometryFormat: "raw",
    	})
...

    {
        ...
        cursor, err := r.Expr(true).And(false).Run(suite.session, r.RunOpts{
			GeometryFormat: "raw",
    	})
...

    {
        ...
        cursor, err := r.Expr(false).And(false).Run(suite.session, r.RunOpts{
			GeometryFormat: "raw",
    	})
...

    {
        ...
        cursor, err := r.And(true, r.Expr(false)).Run(suite.session, r.RunOpts{
			GeometryFormat: "raw",
    	})
...

    {
        ...
        cursor, err := r.And(false, r.Expr(false)).Run(suite.session, r.RunOpts{
			GeometryFormat: "raw",
    	})
...

    {
        ...
        cursor, err := r.And(true, false).Run(suite.session, r.RunOpts{
			GeometryFormat: "raw",
    	})
...

    {
        ...
        cursor, err := r.And(false, false).Run(suite.session, r.RunOpts{
			GeometryFormat: "raw",
    	})
...

    {
        ...
        cursor, err := r.Expr(true).Or(false).Run(suite.session, r.RunOpts{
			GeometryFormat: "raw",
    	})
...

    {
        ...
        cursor, err := r.Or(true, r.Expr(true)).Run(suite.session, r.RunOpts{
			GeometryFormat: "raw",
    	})
...

    {
        ...
        cursor, err := r.Or(true, r.Expr(false)).Run(suite.session, r.RunOpts{
			GeometryFormat: "raw",
    	})
...

    {
        ...
        cursor, err := r.Or(true, false).Run(suite.session, r.RunOpts{
			GeometryFormat: "raw",
    	})
...

    {
        ...
        cursor, err := r.Expr(false).Or(false).Run(suite.session, r.RunOpts{
			GeometryFormat: "raw",
    	})
...

    {
        ...
        cursor, err := r.Or(false, r.Expr(false)).Run(suite.session, r.RunOpts{
			GeometryFormat: "raw",
    	})
...

    {
        ...
        cursor, err := r.Expr(true).Not().Run(suite.session, r.RunOpts{
			GeometryFormat: "raw",
    	})
...

    {
        ...
        cursor, err := r.Not(true).Run(suite.session, r.RunOpts{
			GeometryFormat: "raw",
    	})
...

    {
        ...
        cursor, err := r.And(true, true).Not().Eq(r.Or(r.Expr(true).Not(), r.Expr(true).Not())).Run(suite.session, r.RunOpts{
			GeometryFormat: "raw",
    	})
...

    {
        ...
        cursor, err := r.And(true, false).Not().Eq(r.Or(r.Expr(true).Not(), r.Expr(false).Not())).Run(suite.session, r.RunOpts{
			GeometryFormat: "raw",
    	})
...

    {
        ...
        cursor, err := r.And(false, false).Not().Eq(r.Or(r.Expr(false).Not(), r.Expr(false).Not())).Run(suite.session, r.RunOpts{
			GeometryFormat: "raw",
    	})
...

    {
        ...
        cursor, err := r.And(false, true).Not().Eq(r.Or(r.Expr(false).Not(), r.Expr(true).Not())).Run(suite.session, r.RunOpts{
			GeometryFormat: "raw",
    	})
...

    {
        ...
        cursor, err := r.And(true, true, true, true, true).Run(suite.session, r.RunOpts{
			GeometryFormat: "raw",
    	})
...

    {
        ...
        cursor, err := r.And(true, true, true, false, true).Run(suite.session, r.RunOpts{
			GeometryFormat: "raw",
    	})
...

    {
        ...
        cursor, err := r.And(true, false, true, false, true).Run(suite.session, r.RunOpts{
			GeometryFormat: "raw",
    	})
...

    {
        ...
        cursor, err := r.Or(false, false, false, false, false).Run(suite.session, r.RunOpts{
			GeometryFormat: "raw",
    	})
...

    {
        ...
        cursor, err := r.Or(false, false, false, true, false).Run(suite.session, r.RunOpts{
			GeometryFormat: "raw",
    	})
...

    {
        ...
        cursor, err := r.Or(false, true, false, true, false).Run(suite.session, r.RunOpts{
			GeometryFormat: "raw",
    	})
...

    {
        ...
        var expected_ Err = err("ReqlDriverCompileError", "Calling '==' on result of infix bitwise operator:")
        /* r.expr(r.expr(True) & r.expr(False) == r.expr(False) | r.expr(True)) */

    	suite.T().Log("About to run line #143: r.Expr(r.Expr(true).And(r.Expr(false)).Eq(r.Expr(false).Or(r.Expr(true))))")

        cursor, err := r.Expr(r.Expr(true).And(r.Expr(false)).Eq(r.Expr(false).Or(r.Expr(true)))).Run(suite.session, r.RunOpts{
			GeometryFormat: "raw",
    	})
...

    {
        ...
        cursor, err := r.Expr(r.And(true, false).Eq(r.Or(false, true))).Run(suite.session, r.RunOpts{
			GeometryFormat: "raw",
    	})
    ...
    {
        // math_logic/logic.yaml line #148
        /* err("ReqlDriverCompileError", "Calling '>=' on result of infix bitwise operator:", []) */
        var expected_ Err = err("ReqlDriverCompileError", "Calling '>=' on result of infix bitwise operator:")
        /* r.expr(r.expr(True) & r.expr(False) >= r.expr(False) | r.expr(True)) */

    	suite.T().Log("About to run line #148: r.Expr(r.Expr(true).And(r.Expr(false)).Ge(r.Expr(false).Or(r.Expr(true))))")

        cursor, err := r.Expr(r.Expr(true).And(r.Expr(false)).Ge(r.Expr(false).Or(r.Expr(true)))).Run(suite.session, r.RunOpts{
			GeometryFormat: "raw",
    	})
...

    {
        ...
        cursor, err := r.Expr(r.And(true, false).Ge(r.Or(false, true))).Run(suite.session, r.RunOpts{
			GeometryFormat: "raw",
    	})
...

    {
        ...
        cursor, err := r.Expr(1).And(true).Run(suite.session, r.RunOpts{
			GeometryFormat: "raw",
    	})
...

    {
        ...
        cursor, err := r.Expr(false).Or("str").Run(suite.session, r.RunOpts{
			GeometryFormat: "raw",
    	})
...

    {
        ...
        cursor, err := r.Expr(1).Not().Run(suite.session, r.RunOpts{
			GeometryFormat: "raw",
    	})
...

    {
        ...
        cursor, err := r.Expr(nil).Not().Run(suite.session, r.RunOpts{
			GeometryFormat: "raw",
    	})
